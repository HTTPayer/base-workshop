/*
Simple client script to call x402 endpoint through HTTPayer relay (same-chain, privacy-preserving)
*/
...
async function testRelay() {
  ...
  const RELAY_URL = "https://api.httpayer.com/relay";
  ...
  logInfo("Search term", payload.data.search_term);
  ...
  try {
    ...
    if (firstResponse.status === 402) {
      ...
      const savedFile = await saveResponse(paidResponse, "heurist-search", paymentResponse, {
        prefix: "demo02"
      });
      ...
      if (contentType?.includes("application/json")) {
        ...
        logResponseBody(body, "Heurist AI Search Results", savedFile);
      } else {
        const text = await paidResponse.text();
        logResponseBody(text, "Heurist AI Search Results", savedFile);
      }
